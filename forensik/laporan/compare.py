...
from ..metrics import full_metrics, mcnemar, prior_matched_threshold
...
if pvals:
    keys = list(pvals)
    adjusted = holm([pvals[k] for k in keys])
    adj = {
        k: {"p_raw": pvals[k], "p_adj": p, "significant": p < 0.05}
        for k, p in zip(keys, adjusted, strict=False)
    }
    ...
    for k in sorted(adj, key=lambda k: adj[k]["p_raw"]):
        ...
        out(f"| {k} | {d['only_a']} | {d['only_b']} | {adj[k]['p_raw']:.4g} | "
...
open(os.path.join(HERE, "docs", "hasil", "PERBANDINGAN.md"), "w", encoding="utf-8").write("\n".join(L))
