...
from ..models.data import load_manifest, make_splits
...
files = [os.path.join(d, "test_scores.npy")
         for d in sorted(glob.glob(os.path.join(HERE, "runs", "hubert*"))) if "b16e10" in d]
...
for k, i in enumerate(wrong, 1):
    ...
    details.append({"fname": r["fname"], "truth": r["cls"], "pred": "fake" if pred[i] else "real",
...
open(os.path.join(HERE, "docs", "hasil", "HASIL_ANALISIS_ERROR.md"), "w", encoding="utf-8").write("\n".join(L))
