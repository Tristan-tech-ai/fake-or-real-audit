"""Fake-or-Real 2sec: manifest, splits, and augmentation.

Four decisions differ from the proposal. The official split is the default, since
a random 60/20/20 destroys the domain separation the dataset is built around.
Loudness normalisation replaces peak normalisation, which amplifies an RMS
shortcut. Codec augmentation is applied to both classes equally, neutralising the
MP3-to-fake correlation present in training but absent in test. Nothing depends
on librosa or numba.
"""
