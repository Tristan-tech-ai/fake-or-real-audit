...
Rules applied to every figure, because the previous set failed on all of them:
constrained layout so labels never clip, legends outside the data area beyond
three entries, point labels nudged apart by local density, error bars whenever
more than one seed exists, and no three-dimensional projections.
"""
import glob
import json
import os
...
def gambar1():
    ...
    for i, (kelas, warna) in enumerate([("asli", C["asli"]), ("palsu", C["palsu"])]):
        ...
        b = ax.bar(x + (i - 0.5) * w, pct, w, color=warna, zorder=3,
                   label=f"kelas {kelas}")
        for xi, v in zip(x + (i - 0.5) * w, pct, strict=False):
...
def gambar2():
    ...
    ax.annotate("", xy=(1.0, o["a05"].mean()), xytext=(1.0, o["apm"].mean()),
                arrowprops={"arrowstyle": "<->", "color": C["aksen"], "lw": 2})
...
def gambar3():
    ...
    if o and r:
        ...
        for i, (nm, vals, cl) in enumerate([
            ...
            for xi, v in zip(x2 + (i - 1) * w, vals, strict=False):
...
def gambar4():
    ...
    for ax, (m, b, judul) in zip(axs, ARS4, strict=False):
        nm, mu, er, cl, nn = [], [], [], [], []
        ...
        ax.barh(y, mu, xerr=er, color=cl, height=0.62, zorder=3,
                error_kw={"ecolor": C["gelap"], "elinewidth": 1.2, "capsize": 4})
        for i, (v, e, n) in enumerate(zip(mu, er, nn, strict=False)):
...
def gambar6():
    ...
    for (nama, aug, v, cl), yy in zip(kurva, pos, strict=False):
        ax.annotate(f"{nama} [{aug}]", (xs[-1], v[-1]),
                    xytext=(xs[-1] + 0.35, yy), fontsize=9, color=cl,
                    weight="bold", va="center",
                    arrowprops={"arrowstyle": "-", "color": cl, "lw": 0.9,
...
FIGURES = (gambar1, gambar2, gambar3, gambar4, gambar5, gambar6, gambar7,
           gambar8, gambar9)
...
def main() -> None:
    ...
    for figure in FIGURES:
        try:
            ...
            print(f"  !! {figure.__name__} gagal: {type(e).__name__}: {e}")
