"""Train and evaluate a detector on Fake-or-Real 2sec.

    py -m forensik.latih --model wavlm --split official --augment full --epochs 10

Differs from the proposal configuration in six ways: per-model learning rates
instead of a uniform 1e-3, AdamW with warmup and cosine decay, early stopping on
validation EER, bf16 mixed precision, label smoothing, and a decision threshold
tuned on validation rather than fixed at 0.5.
"""
...
from .models.models import DEFAULT_LR, build_model
...
def main():
    ...
    for ep in range(1, args.epochs + 1):
        ...
        eer_v, _ = equal_error_rate(yv, pv)
    ...
    print("  --- ambang prior-matched (transduktif, tanpa label) ---")
