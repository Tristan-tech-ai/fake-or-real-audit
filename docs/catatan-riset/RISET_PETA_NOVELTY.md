...
| Berkas | Isi |
|---|---|
| (jalur lokal, dihapus) | Audit empiris dataset, dokumen paling penting |
| (jalur lokal, dihapus) | Tabel hasil + koreksi multi-seed + ablasi 2×2 |
| (jalur lokal, dihapus) | 4 arsitektur × 3 seed, McNemar, korelasi error, ensemble |
| (jalur lokal, dihapus) | Verifikasi protokol split ref [13]/[19]/[20] |
| (jalur lokal, dihapus) | AUC 18 fitur trivial, resolusi statistik |
| (jalur lokal, dihapus) | Energi >6 kHz per split × kelas × provenance |
| (jalur lokal, dihapus) | Profil 20 pita, cutoff spektral |
| (jalur lokal, dihapus) | Eksperimen penentu split resmi vs acak |
| (jalur lokal, dihapus) | **Lokasi bug P0-1** (RNG augmentasi beku, baris ~293) |
| (jalur lokal, dihapus) | **Lokasi bug kapasitas** (`SSL_CKPT`, baris ~88–92) |
| (jalur lokal, dihapus) | EER, DET, McNemar, Holm-Bonferroni, ECE, temperature scaling, prior-matched threshold |
| (jalur lokal, dihapus) | 24 run tersimpan, `results.json` + `test_scores.npy` per run |
| (jalur lokal, dihapus) | Harness evaluasi `for-rerec` (816 berkas sudah terekstrak) |